...
if iex ==1
    ...
    area1 = h/2*(y_sum(1)+y_sum(10)+2*(y_sum(2)+y_sum(3)+y_sum(4)+y_sum(5)+y_sum(6)+y_sum(7)+y_sum(8)+y_sum(9))); % KEY
...
if iex ==2
    ...
    % Discretize X for summation
    xmin = 0; xmax = 4;
    Nsamples = 50; % Number of points for dividing x (N-1 rectangles)
    x_sum = linspace(xmin,xmax,Nsamples);
    h = x_sum(2) - x_sum(1);
